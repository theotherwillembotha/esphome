...
optional<DscData> DscProtocol::decode(RemoteReceiveData src) {
  // OOK RF receivers often capture spurious noise alongside the actual packet,
  // and a buffer may contain multiple marks ≥ DSC_PREAMBLE_MIN_US. We try
  // each such mark as a preamble candidate and return on the first valid packet.
  //
  // Use direct comparison (not peek_mark_at_least) so that tolerance is NOT
  // applied — peek_mark_at_least(2000) with 25% tolerance has a lower bound
  // of 1500 µs, causing short noise marks to be mistaken for preambles.
  while (src.is_valid()) {
    // Find the next preamble candidate
    ...
    // Keep a copy of src positioned at the preamble so the outer scan can
    // advance past it and retry from the next candidate on failure.
    RemoteReceiveData decode_src = src;
    src.advance();       // outer scan moves past this preamble
    decode_src.advance();  // decode attempt starts at the element after the preamble
    ...
    if (!decode_src.is_valid()) {
      continue;
    }
    int32_t first_space = decode_src.peek();
    if (first_space >= 0) {
      continue;  // Expected a space after the preamble, got a mark
    }
    ...
    while (decode_src.is_valid() && bit_pos < 48) {
      int32_t space_val = decode_src.peek();
      if (space_val >= 0) {
